...
public class DslBasketRepository {
    ...
    public List<BasketDto> userBaskets(Integer userId) {
        return jpaQueryFactory
                .select(new QBasketDto(basket.users.id, users.username, basket.id, product.productName, product.productPrice, product.productImage, product.productDescription, basket.count, product.productEtc, product.productPrice.multiply(basket.count)))
                .from(basket)
